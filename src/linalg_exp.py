"""
Experimental version of linalg package

This seperate version allows for experimental uses such as tweaking
precision of the individual options. This way the standard version
doesn't have to try to be everything and include lots of extra 
functionality that won't be used and may just get in the way.
...
def gedo(a, pivot=True):
    ...
    for j in range(0,n):    
        ...
        for i in range(j+1,n):
            try:
                a[ov[i],j] = round(a[ov[i],j]/a[ov[j],j], precision)
        ...
        for i in range(j+1,n): #row number
            for k in range(j+1,n): #column number
                ...
                a[ov[i],k] = round(a[ov[i],k] - round(a[ov[i],j]*a[ov[j],k], precision), precision)
...
def dofe(lu, ov, b):
    ...
    for j in range(n):
        for i in range(j+1,n):
            c[ov[i]] = round(c[ov[i]] - round(lu[ov[i],j]*c[ov[j]], precision), precision)
...
def dobs(lu, ov, c):
    ...
    #back substitution
    x[ov[mmo]] = round(c[ov[mmo]]/lu[ov[mmo],mmo], precision) # last row solution
    for j in range(mmo,-1,-1):
        x[ov[j]] = c[ov[j]] #initialize solution value
        for k in range(mmo,j,-1):
            #group known terms in numerator
            x[ov[j]] = round(x[ov[j]] - round(x[ov[k]]*lu[ov[j],k], precision), precision)
        x[ov[j]] = round(x[ov[j]]/lu[ov[j],j], precision) #solving equation by division
...
def tdfe(lu, b):
    """
    Doolittle reduction forward elmination applied to the b vector of a 
    tridiagonal system.
...
def gecr(a, pivot=True):
    """Generic square matrix LU Decomposition using Crout algorithm with
    partial pivoting.
